...
public class Gallery_Intiator extends Activity{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(Base.isAudConvert){
			Intent picker = new Intent(Intent.ACTION_PICK, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI);
			startActivityForResult(picker,1);
		}
		else{
			Intent picker = new Intent(Intent.ACTION_PICK, MediaStore.Video.Media.EXTERNAL_CONTENT_URI);
