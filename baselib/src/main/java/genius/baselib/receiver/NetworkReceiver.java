...
public class NetworkReceiver  extends BroadcastReceiver {
    ...
    public static final boolean checkEnable = false;
        ...
        switch (TempData.getWeak(PreferenceUtil.getValue(context, NetworkReceiver.NetworkStatusKey,-1))){
            ...
            case 0:
                ...
                if(NetworkReceiver.checkEnable){
                    BusTool.sendBus(new BusBuilder(BusBuilder.BUSTYPE.onEventMainThread,true, UtilsNetwork.TYPE.NOT_CONNECTED,NetworkReceiver.UPDATE_NET));
                }

                break;
            case 1:
                if(NetworkReceiver.checkEnable){
                    BusTool.sendBus(new BusBuilder(BusBuilder.BUSTYPE.onEventMainThread,true,UtilsNetwork.TYPE.WIFI,NetworkReceiver.UPDATE_NET));
                }
                break;
            case 2:
                if(NetworkReceiver.checkEnable){
                    BusTool.sendBus(new BusBuilder(BusBuilder.BUSTYPE.onEventMainThread,true,UtilsNetwork.TYPE.MOBILE,NetworkReceiver.UPDATE_NET));
                }
                break;
        }
    }
    ...
    private static void noSetCheck(Context context) {
        ...
        if(NetworkReceiver.checkEnable){
            BusTool.sendBus(new BusBuilder(BusBuilder.BUSTYPE.onEventMainThread,true,connectivityStatus,NetworkReceiver.UPDATE_NET));
        }
    }
